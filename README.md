...
Notes: 
- For directories, ``/compile`` and ``/output`` are required. The ``/config`` is optional.
...
- ``NPROC``: To define how many thread(s) will be used while compling.   
- Use ``singlethread=True`` environment variable to force compile in single thread mode.  
